...
import (
	...
	metric "github.com/mia0x75/gopfc/metric"
)
...
func basic() {
	for _ = range time.Tick(time.Second * time.Duration(10)) {
		...
		metric.Meter("test.meter", pv)
		metric.Meter("test.meter.2", pv-50)

		// (常用) Gauge,用于保存数值类型的瞬时记录值。使用场景如，统计队列长度、统计CPU使用率等
		queueSize := int64(rand.Int()%100 - 50)
		metric.Gauge("test.gauge", queueSize)

		cpuUtil := float64(rand.Int()%10000) / float64(100)
		metric.GaugeFloat64("test.gauge.float64", cpuUtil)
	}
}
...
func senior() {
	for _ = range time.Tick(time.Second) {
		...
		metric.Histogram("test.histogram", delay)
	}
}
